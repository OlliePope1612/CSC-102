...
SHOW_BUTTONS = False   # show Pause/Quit buttons in GUI (only matters in mock)
COUNTDOWN = 100       # initial countdown (seconds)
...
# pick a random LED color for the button: R, G or B
button_color = choice(["R","G","B"])
# decide what “release-to-defuse” digit is required
if button_color == "G":
    # green → defuse when seconds matches first digit of serial
    button_target = next(ch for ch in serial if ch.isdigit())
elif button_color == "B":
    # blue → defuse when seconds matches last digit of serial
    button_target = next(ch for ch in reversed(serial) if ch.isdigit())
else:
    # red → simple press to defuse
    button_target = None
...
if DEBUG:
    ...
    print(f"Button color:   {button_color}, target: {button_target}")
